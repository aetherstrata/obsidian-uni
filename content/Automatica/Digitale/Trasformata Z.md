...
$$
\begin{align*}
x(t) = \sum*{k=0}^\infty x[k]\delta_0(t-kT_c) &\quad\overset{\mathcal{L}}{\longrightarrow}\quad X(S) = \sum*{k=0}^\infty x[k]e^{-skT*c}\\
\{x_k\} &\quad\overset{Z}{\longrightarrow}\quad X(Z) = \sum*{k=0}^\infty x[k]z^{-k}
\end{align*}
$$
...
$$
\lim*{z\to1}\left(1-z^{-1}\right)Y(z) = \lim*{k\to\infty}\left\{y_k\right\}
$$
